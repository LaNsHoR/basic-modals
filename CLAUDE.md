...
Styles live in `src/style.js` as a JS object and are injected via `CSS({ '@layer basic-modals': style })` from `html-css-builder` (the sole runtime dependency) — i.e. wrapped in a named cascade layer. Injection is **lazy and guarded**: `inject_styles()` runs once on the first modal open and is a no-op when `document` is undefined, so importing the module in a non-DOM environment (SSR: plain Node, Next, Remix) doesn't crash. The `@layer` is **load-bearing, not cosmetic**: because injection is lazy, this stylesheet lands *after* a consumer's own stylesheet, so equal-specificity overrides of `.BasicModals*` would otherwise lose on source order; an unlayered rule always outranks a layered one, so the layer keeps the defaults overridable regardless of injection order or specificity — don't remove it. The browser global `window.BasicModals` is likewise assigned only when `window` exists. The fade relies on `@starting-style` and `transition ... allow-discrete`, and the `@layer` wrapper is itself a nested at-rule — which is why `html-css-builder >= 0.2.0` is required. Selectors are plain CSS strings (camelCase property keys); nesting like `@starting-style` is expressed as a nested object.
